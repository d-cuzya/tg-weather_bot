...
std::string WeatherApi::getCurrentWeatherByCord(const double& latitude, const double& longitude) {
    std::string response;
    CurlEasy::getInstance()->createRequest(this->url + "weather?lat=" + std::to_string(latitude) +"&lon="+ std::to_string(longitude) +"&appid=" + std::getenv("WEATHERAPI_KEY") + "&lang=ru&cnt=6&units=metric",response);
    return response;
}

std::string WeatherApi::getForecastWeatherByCord(const double& latitude, const double& longitude) {
    std::string response;
    CurlEasy::getInstance()->createRequest(this->url + "forecast?lat="+ std::to_string(latitude) +"&lon="+ std::to_string(longitude) +"&appid=" + std::getenv("WEATHERAPI_KEY") + "&lang=ru&cnt=6&units=metric", response);
    return response;
}
